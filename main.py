...
from catalyst.api import (record, order, symbol, get_open_orders)
    ...
    if exit_currency in exit_balances:
        ...
        log.warn('the selling exchange {exchange_name} does not hold '
                 'currency {currency}'.format(
                     exchange_name=exit_exchange.name, currency=exit_currency))
        return
...
def handle_data(context, data):
    ...
    buying_price = data.current(context.trading_pairs[context.buying_exchange],
                                'price')
    ...
    log.info('the price gap: {gap} ({gap_percent}%)'.format(
        gap=gap, gap_percent=gap * 100))
        ...
        if orders:
            log.info('found {order_count} open orders on {exchange_name} '
                     'skipping bar until all open orders execute'.format(
                         order_count=len(orders), exchange_name=exchange.name))
            return
    ...
    if buy_amount:
        ...
        # Override of wider gap is found
        exit_points = sorted(
            context.exit_points, key=lambda point: point['gap'], reverse=True)
